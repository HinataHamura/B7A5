...
export interface Gear { id: string; name: string; category: string; brand: string; pricePerDay: number; available: boolean; stock: number; image: string; images?: string[]; description: string; specifications?: Record<string,string>; provider?: Pick<User,"id"|"name">; rating: number; reviewCount: number }
