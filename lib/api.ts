...
function normalizeGear(g:RawGear,index=0):Gear{const fallback=`https://images.unsplash.com/${fallbackImages[index%fallbackImages.length]}?auto=format&fit=crop&w=1200&q=80`;const images=g.images?.filter(x=>x.startsWith("https://")&&!x.includes("images.example.com"));return {id:g.id,name:g.name,description:g.description,brand:g.brand||"Independent",pricePerDay:Number(g.pricePerDay),stock:g.stock,available:g.available,images:images?.length?images:[fallback],image:images?.[0]||fallback,category:typeof g.category==="string"?g.category:g.category?.name||"Outdoor",categoryId:typeof g.category==="object"?g.category.id:g.categoryId,provider:g.provider,rating:g.averageRating??0,reviewCount:g.reviewCount??0,specifications:g.specifications?Object.fromEntries(Object.entries(g.specifications).map(([k,v])=>[k,String(v)])):undefined}}
...
export async function getGearForDates(startDate:string,endDate:string){if(demoMode)return demoGear;const query=new URLSearchParams({startDate,endDate});return (await api<RawGear[]>(`/gear?${query}`)).map(normalizeGear)}
